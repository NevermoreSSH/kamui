...
cat /usr/local/etc/xray/tls.json | jq '.inbounds[0].settings.clients += [{"id": "'${uuid}'","flow": "xtls-rprx-direct","email": "'${email}'"}]' > /usr/local/etc/xray/tls.json.tmp
...
linkXTLS="vless://${uuid}@${installSubDomain}:443?security=xtls&encryption=none&type=tcp&sni=${installSubDomain}&flow=xtls-rprx-direct#vless_xtls_${username}"
linkTLS="vless://${uuid}@${installSubDomain}:443?security=tls&encryption=none&type=tcp&sni=${installSubDomain}#vless_tls_${username}"
...
echo "TLS Type : xtls, tls, none (No-TLS)"
